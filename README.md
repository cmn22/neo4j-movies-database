# Create data
<details>
<summary>Run Movies Model Data Cypher</summary>
...
#### Find the actor Tom Hanks which is a Person in the graph?
```sql
MATCH (p:Person {name: "Tom Hanks"})
RETURN p
```

#### Find a movie called Cloud Atlas
```sql
...
## Find List
#### Find all the people (nodes with the Person label) in the graph, but limit the returned number to ten:
```sql
MATCH (p:Person)
RETURN p.name LIMIT 10
```

#### Find movies released in a specific timespan, you can add conditions by using WHERE:
```sql
...
#### Find which movies Tom Hanks have acted in
```sql
MATCH (p:Person {name: "Tom Hanks"})-[:ACTED_IN]->(m:Movie)
RETURN m.title
```

#### Create structure in which:
1. All movies that Tom Hanks has acted in (m).
2.	All co-actors (coActors) who have acted in the same movies as Tom Hanks.
3.	The full path structure connecting

Tom Hanks → Movie → Co-Actors
```sql
...
# Six Degrees
The concept that any two people on Earth are six or fewer connections apart.

#### All movies and people up to six hops away from Kevin Bacon
```sql
MATCH (:Person {name:"Kevin Bacon"})-[*1..6]-(n)
RETURN DISTINCT n
```

#### Find if this theory is true for Kevin Bacon and Meg Ryan
```sql
...
#### Find actors Tom Hanks is yet to work with, but who has worked with his previous co-actors.
```sql
...
#### Who wouldn't love to see Tom Hanks and Keanu Reeves to do a movie together? Let's see who can introduce them.
```sql
...
# Additional Queries
Here are additional Cypher queries that you can use to explore and manipulate the Neo4j database further:
...
RETURN m.title AS MoviesDirected
```

#### Find All Relationships for a Person
```sql
MATCH (p:Person {name: "Tom Hanks"})-[r]->(n)
RETURN type(r) AS RelationshipType, n AS Node
```

&nbsp;

## Graph Exploration
#### Find All Collaborators of a Person
```sql
MATCH (p:Person {name: "Tom Hanks"})-[:ACTED_IN]->(m:Movie)<-[:ACTED_IN]-(collaborators)
RETURN DISTINCT collaborators.name AS Collaborators
```

#### Find All Directors Tom Hanks Has Worked With
```sql
MATCH (p:Person {name: "Tom Hanks"})-[:ACTED_IN]->(m:Movie)<-[:DIRECTED]-(d:Person)
RETURN DISTINCT d.name AS Directors
```

#### List All People Who Directed and Acted in the Same Movie
```sql
MATCH (p:Person)-[:DIRECTED]->(m:Movie)<-[:ACTED_IN]-(p)
RETURN p.name AS MultiRolePerson, m.title AS Movie
```
...
#### Find the Most Popular Movie (By Number of Actors)
```sql
MATCH (m:Movie)<-[:ACTED_IN]-(p:Person)
RETURN m.title AS Movie, COUNT(p) AS ActorCount
ORDER BY ActorCount DESC
```

#### Find the Longest Acting Chain Between Two Actors
```sql
MATCH path=shortestPath(
  (:Person {name: "Tom Hanks"})-[*]-(:Person {name: "Kevin Bacon"})
)
RETURN path
```
...
## Recommendations
#### Recommend a Movie Based on a Person's Co-Actors
```sql
MATCH (p:Person {name: "Tom Hanks"})-[:ACTED_IN]->(m:Movie)<-[:ACTED_IN]-(coActors),
      (coActors)-[:ACTED_IN]->(recommendedMovies)
WHERE NOT (p)-[:ACTED_IN]->(recommendedMovies)
RETURN DISTINCT recommendedMovies.title AS RecommendedMovies
```
...
#### Find the Actor Who Worked With the Most Unique Co-Actors
```sql
MATCH (p:Person)-[:ACTED_IN]->(m:Movie)<-[:ACTED_IN]-(coActors)
RETURN p.name AS Actor, COUNT(DISTINCT coActors) AS UniqueCoActors
ORDER BY UniqueCoActors DESC
``` 

&nbsp;

## Updating the Graph
#### Add a New Movie Node
```sql
CREATE (:Movie {title: "New Movie", released: 2024, tagline: "An exciting new film"})
```

#### Create a Relationship Between an Actor and a Movie
```sql
MATCH (p:Person {name: "Keanu Reeves"}), (m:Movie {title: "Cloud Atlas"})
CREATE (p)-[:ACTED_IN {roles: ["Main Character"]}]->(m)
```

#### Update Properties on a Node
```sql
MATCH (m:Movie {title: "Cloud Atlas"})
SET m.tagline = "A breathtaking journey through time"
RETURN m
```
...
#### Return a Subgraph of All Movies Released After 2000
```sql
MATCH path=(:Movie {released: 2000})-[:ACTED_IN*1..2]-()
RETURN path
```

#### Visualize Connections Between Co-Actors
```sql
MATCH path= (:Person {name: "Tom Hanks"})-[:ACTED_IN]->(m)<-[:ACTED_IN]-(coActors)
RETURN path
```
